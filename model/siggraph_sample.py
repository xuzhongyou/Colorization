...
class SIGGRAPHGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, use_tanh=True, classification=True):
        ...
        if(use_tanh):
            model_out+=[nn.Tanh()]
        predict_sample = [nn.Conv2d(512,512,3,1,1)]
        predict_sample += [nn.Conv2d(512,60,1,1,0)]
        ...
        self.predict_sample = nn.Sequential(*predict_sample)
    ...
    def forward(self, x):
        ...
        predict_sample = self.predict_sample(conv4_3.detach())
...
def active_layer(temp_names,net):
    for i,(name, param) in enumerate(net.named_parameters()):
        if name in temp_names:
            print('activate layer name',name)
            param.requires_grad = True
    return net

def show_nameparm(net):
    for i,(name, param) in enumerate(net.named_parameters()):
        
        print('{} grad: {}'.format(name,param.requires_grad))
    return net

def transfer_layer(temp_names,net):
    for i,(name, param) in enumerate(net.named_parameters()):
        if name in temp_names:
            print('activate layer name',name)
            param.requires_grad = True
...
if __name__ == "__main__":
    temp_names = ['predict_sample.0.weight','predict_sample.0.bias','predict_sample.1.weight','predict_sample.1.bias']
    net = SIGGRAPHGenerator(4,2)
    print(net)
    input = torch.rand((4,4,176,176))
    clsout,regout = net(input)
    print(clsout.shape,regout.shape)
    net = transfer_layer(temp_names,net)
    net = show_nameparm(net)
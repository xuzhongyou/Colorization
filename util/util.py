...
def get_colorization_data(data_raw, opt, ab_thresh=5., p=.125, num_points=None):
    """this function aims to remove thee grayscale image in dataset"""
...
#   according to the training code,the mask_B is always 0 with out centralization and hint_mask
    ...
    for nn in range(N):
        ...
        while(cont_cond):
            ...
            P = np.random.choice(opt.sample_Ps) # patch size [1, 2, 3, 4, 5, 6, 7, 8, 9, ]
            if flag is True:
                print('---------------P is ',P)
            # sample location
            ...
            if(use_avg):
                # embed()
                if flag is True:
                    a = torch.mean(data['B'][nn,:,h:h+P,w:w+P],dim=2,keepdim=True)
...
def crop_mult(data,mult=16,HWmax=[800,1200]):
    ...
    # pay attention to the divide /  and //
    H,W = data.shape[2:]
...
def encode_ab_ind(data_ab, opt):
    ...
    flag = False
    if flag is True:
        print('data_ab-----------------',data_ab.shape,data_ab) # the shape is [5,2,44,44]
    data_ab_rs = torch.round((data_ab*opt.ab_norm + opt.ab_max)/opt.ab_quant) # normalized bin number
    # torch.round return the cloest integer
    if flag is True:
        print('data_ab_rs----------------',data_ab_rs.shape,data_ab_rs)
    data_q = data_ab_rs[:,[0],:,:]*opt.A + data_ab_rs[:,[1],:,:]
    if flag is True:
        print('data_q------------------',data_q) # the shape is [5,1,44,44]
    return data_q
